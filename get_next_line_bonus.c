...
##appending the list##
	...
	DIFFERENCE(S) with the MANDATORY PART:
	additional parameter fd(bonus) vs none(mand)
*/
void	append_list(t_list **list, char *buffer, int fd)
{
	t_list	*new_node;
	t_list	*last_node;
...
Creating the linked list:
	...
	-head node pointing to the remaining part of the last buffer 
		which belongs to the next line
	-freeing the memory of the nodes 
	relevant to the previous line
...
void	create_list(t_list **list, int fd)
{
	...
	while (!find_new_line(list[fd]))
	{
		...
		buffer[char_read] = '\0';
		append_list(list, buffer, fd);
	}
}

/* 
	DIFFERENCE(S) with the MANDATORY PART:
	- array of t_list ponters (bonus) vs a point (mand)
	- check if fd > 4095
*/
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE <= 0 || fd < 0 || fd > 4095 || read(fd, &next_line, 0) < 0)
		return (NULL);
	create_list(list, fd);
	if (list[fd] == NULL)
		return (NULL);
	next_line = full_line(list[fd]);
	rearrange_list(&list[fd]);
	return (next_line);
}
